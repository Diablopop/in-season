...
|---|---|---|---|
| D1 | Total duration | **1000ms** — 850ms hold, 150ms fade, eased out. Settled by feel on a phone across several passes rather than by argument, and it landed above the ~1s guidance Claude opened with. The split turned out to matter more than the total: dwell time on the artwork is the drama, and the fade contributes none of it — long fades read as lag, not atmosphere. A true hard cut is `--cover-fade: 1ms`. | Claude argued for ~900ms on the Nielsen threshold, where a pause stops reading as a transition. That was the right starting point and the wrong conclusion: the threshold describes waiting for something, and this is not a wait. Exposed as two custom properties, so any other answer is a one-line change. |
...
**Milestone 1 verified 2026-08-22.** The animation was seeked frame by frame rather than timed against the wall clock: opacity holds at 1.00 through the hold, then eases to 0.00 at the end, and `visibility` flips to hidden at the end. Fonts fetch at 14ms and complete at 16ms, so the serif is available well before the hold expires. **Device verification complete 2026-08-23.** Andrew confirmed the installed-PWA cold launch, and that Reduce Motion suppresses the cover entirely — no cream screen, straight to the grid. That closes the last thing Claude could not observe: the mechanism was already proven on desktop, which takes the identical `display: none` path, but the iOS mapping from the OS setting to `prefers-reduced-motion` had not been watched on a real device.
